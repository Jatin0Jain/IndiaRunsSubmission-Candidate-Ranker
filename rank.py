...
REFERENCE_DATE = date.today()  # Always relative to when ranking is run
...
# CONSULTING_FIRMS imported from constants.py
AI_SKILLS = {
...
# PHASE 2: DEEP SCORER
# =======================================================
# SKILL_RELEVANCE, PROFICIENCY_MULT imported from constants.py
...
def score_candidate_full(cid, full_json):
    ...
    if title_score == 0.0:
        # Soft floor instead of hard zero — candidate passed Phase 1, so give
        # them a minimum base and let the other components decide their rank.
        title_score = 5.0
    score += title_score
...
def generate_reasoning(cid, full_json, rank, score, pre_generated):
    ...
    gh_str = f"; GitHub activity score: {gh:.0f}" if gh > 50 else ""
    
    return (
        f"{title} with {yoe:.1f} yrs exp at {company} ({country}); "
        f"strong in {skills_str}{concern_str}{gh_str}."
    )
